...
| `python -m pytest -q` | 1017 тестов, зелёные, около четырёх минут |
| `./gradlew test` | 522 теста, зелёных, все пять модулей |
...
в модели и в ревизии теперь `OPEN`, и два теста держат это: один читает колонку
сырым SQL, второй вставляет строку **без** колонки и читает её через ORM.
