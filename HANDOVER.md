...
режим приёма у класса, — база дошла до `0011`, и закрылись находки одиннадцати
аудитов, включая три последних: по Android, по боту и по документации.
...
Поверх этого в `dev` легли шесть содержательных коммитов:
...
Ворота:  ruff чисто, 1024 серверных теста, 528 Android, обе сборки (локально)
...
cd server  && python -m pytest -q                        # 1024 теста, ~3.5 мин
cd android && ./gradlew test                             # 528 тестов
...
**Аудитов на этой ветке было одиннадцать.** Четыре по ранней работе
(шестнадцать находок), четыре по `8ec2e31` (шесть), два по `ce02348`
(пятнадцать) и один по `59c7a77` (семь); плюс три последних — по Android, по
боту и по документации — закрыты в коммите, который этот файл и переписал.
Ранняя самая тяжёлая: Самая тяжёлая:
...
- **Режим приёма никто не переключал на живом классе.** Покрыт тестами:
