...
Последнее обновление: **19 сентября 2026**, после мерджа PR #47, #48, #49 и #50.
`main` и `dev` сошлись на `22399e9`; база на голове `0013`; бой отвечает
`{"status":"ok","schema":"0013"}`. Двумя порциями раньше: публикация репозитория
...
**PR #50 смерджен**, пять коммитов; миграция ему не потребовалась — модель не
менялась, голова осталась `0013`. Первые два коммита — режим исправления
...
сделано и что осталось), а конфигурация под агентов — в `.claude/` (см. раздел 6,
«Конфигурация агентов лежит в `.claude/`»). Считайте всё ниже проверенным фактом на указанную дату,
...
### Конфигурация агентов лежит в `.claude/`

Её не было вовсе — ни настроек, ни агентов, ни скиллов. Теперь есть, и устроена
она так, чтобы не пересказывать `CLAUDE.md`, а описывать форму: кто чем владеет,
какие проверки настоящие и какие команды из сессии не запускают никогда.

```
.claude/settings.json   разрешения, один хук, известный маркетплейс
.claude/agents/         восемнадцать агентов по областям
.claude/skills/         девять процедур
.claude/commands/       /where-are-we и /pre-push
.claude/README.md       что здесь лежит и чего здесь намеренно нет
AGENTS.md               указатель для агентов, которые читают не `CLAUDE.md`
.github/copilot-instructions.md   короткий файл: его читают на каждый запрос
```

Восемнадцать агентов, а не сотня, потому что каждый файл существует ради ловушки,
которая в этой области уже сработала, и несёт факт, который бы её предотвратил.
Агент, пересказывающий `CLAUDE.md`, не пережил бы то же ревью, которое здесь не
пропускает комментарий, пересказывающий строчку под собой, а агент-заглушка хуже
отсутствия агента: он уверенно отвечает из ничего. Девятичастное деление из
`skills/audit/SKILL.md` — то самое, которое нашло тридцать одну находку за проход;
остальные девять — области, у которых с тех пор завелись свои ловушки.

Три вещи в `settings.json`, о которых стоит знать:

* **`deny` сравнивает начало строки команды.** `DATABASE_URL=… .venv/bin/alembic
  upgrade head` мимо правила `alembic upgrade *` проходит. Это перила, а не
  забор; причина не запускать её руками — в `skills/migration/SKILL.md`.
* **Хук ровно один**, и он только печатает: на запись в `values/strings*.xml`
  любого модуля напоминает про близнеца в `values-en/`. Закоммиченный хук
  выполняется у каждого, кто клонировал репозиторий, — поэтому он один и поэтому
  он не может упасть.
* **`extraKnownMarketplaces` регистрирует `anthropics/skills`**, но ни один плагин
  не включён: включить — решение за всех, кто клонирует репозиторий, а не за
  сессию, которая завела файл.

**`.mcp.json` намеренно нет.** Подключения к Neon и Vercel требуют учётных данных,
а секреты в этот репозиторий не попадают. Проект Neon — тот, что **называется
`lessons`** (в аккаунте их два), и это всё, что можно записать безопасно.
...
1. Прочитать `CLAUDE.md` целиком, затем `docs/architecture.md`. Если вы агент —
   посмотреть, есть ли в `.claude/agents/` файл вашей области и в
   `.claude/skills/` процедура под задачу: там записаны ловушки, которые в этой
   области уже срабатывали.
