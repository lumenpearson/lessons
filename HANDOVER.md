...
Последнее обновление: **16 сентября 2026**, в тот момент, когда `dev` влит в
...
**Что из списка «названо, но не сделано» осталось:** `If-None-Match` на
клиенте (нужно решить, двигает ли `304` отметку «обновлено N назад» и дёргает
ли он виджет) и летнее окно синхронизации, где приложение просит потянуть
вниз, а тянуть нечего (`SyncWindowTest` фиксирует нынешнее поведение
намеренно). Оба — решения владельца, а не правки.
...
Ворота:  ruff чисто, mypy чисто, 1340 серверных тестов, 568 Android, обе сборки
...
cd server  && python -m pytest -q                        # 1340 тестов, ~5 мин
cd server  && python -m mypy                              # чисто, 79 модулей
cd android && ./gradlew test                             # 568 тестов
...
**Состояние базы на сейчас:** голова Neon — `0012`, `EXPECTED_REVISION` —
`0013`. Значит `/api/v1/warmup` будет звать базу отстающей, пока не применена
`0013`. Это константа говорит правду, а не поломка.
...
1а. **Применить `0013` через коннектор Neon — ПОСЛЕ мерджа PR #45, не до.**
   Это единственная ревизия в цепочке, которая мигрирует в обратную сторону:
   `UNIQUE` ломает **старый** код, а не новый (раздел 6, «Ограничение мигрирует
   не в ту сторону»). До неё `/api/v1/warmup` честно зовёт базу отстающей,
   потому что `EXPECTED_REVISION` уже `0013`. На проде сейчас 0 строк домашки и
   0 дубликатов, так что её `DELETE` не тронет ничего.
