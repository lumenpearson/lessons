...
База:    прод на `0011` — `0010` и `0011` применены через коннектор Neon
         ДО мерджа, как требует порядок; отдельных действий с базой не осталось
```
...
1. **Решить судьбу PR #43** (`dev` → `main`; он черновик — снять с черновика и
   ...
   **С базой перед мерджем делать нечего: она уже на `0011`.** `0010` (режим
   приёма и таблица личных кодов) и `0011` (две отметки времени в
   `diary_overrides` до `NOT NULL`) применены через коннектор Neon в этой
   сессии, до мерджа, как того требует порядок. Ни одна ничего не разрушила:
   `0010` добавила колонку со значением по умолчанию, сохраняющим нынешнее
   поведение, и пустую таблицу; `0011` на нашей базе не изменила ничего вовсе —
   колонки уже были `NOT NULL`, потому что DDL для `0009` брался из модели, а не
   из текста ревизии. Проверено после: голова `0011`, единственный класс читает
   `OPEN`, `device_invites` совпадает с моделью колонка в колонку, три индекса.
   Пока `dev` не влит, база **впереди** кода — это то самое нормальное окно, и
   `/api/v1/warmup` будет честно отвечать «База впереди кода».
