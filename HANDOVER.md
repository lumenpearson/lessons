...
**Веток теперь две: `main` и `dev`.** Вся несделанная работа собрана в `dev`:
рабочая ветка (бывший PR #35, четырнадцать коммитов) и пять веток dependabot —
cryptography, asyncpg, AGP, группа androidx и обёртка Gradle. В `main` не влито
ничего; `dev` — единственное место, где всё это существует вместе.

```
Ветка:   dev  (всё, что не в main)
База:    main (движется сам — dependabot)
Слито:   claude/essentials-design-system-5ttyh6 (PR #35) + PR #36, #38, #39, #41, #42
