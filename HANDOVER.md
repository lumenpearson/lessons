...
Последнее обновление: **15 сентября 2026**, после того как в `dev` легли два
из трёх невзятых пунктов — несколько классов на телефоне и правки поверх
дневника — и шесть дефектов, которые аудит нашёл в первом из них.
...
Из раздела 3.1 остался один пункт — два режима при приглашении, и по нему
сначала нужна постановка от владельца.
...
CI:      был зелёным целиком на a1bbeb5 — Server, Android, What changed, Vercel;
         на 8ec2e31 перепроверьте сами
Ворота:  ruff чисто, 956 серверных тестов, 491 Android, обе сборки (локально на ce02348)
```
...
cd server  && python -m pytest -q                        # 956 тестов, ~3.5 мин
cd android && ./gradlew test                             # 491 тест
...
1. **Решить судьбу PR #43** (`dev` → `main`; он черновик — снять с черновика и
   влить). Миграции применены по `0009` включительно, дополнительных действий с
   базой перед мерджем нет. До мерджа dependabot будет носить свои бампы на `main` заново.
