...
2. **Старые артефакты не удалены.** 500 МБ аккаунта были выбраны APK по 32 МБ
   за прогон, и пока они лежат, загрузки падают (`Artifact storage quota has
   been hit`). Истекают в декабре; удалить раньше — только руками или через
   `gh api -X DELETE .../actions/artifacts/{id}`.
