...
Последнее обновление: **15 сентября 2026**, после того как в `dev` лёг первый
из трёх невзятых пунктов — несколько классов на одном телефоне.
...
Поверх этого в `dev` лёг `8ec2e31` — телефон теперь держит несколько классов
сразу и переключается между ними. Это первый пункт из раздела 3.1; два
оставшихся не начинались.
...
Ворота:  ruff чисто, 921 серверный тест, 466 Android, обе сборки (локально на 8ec2e31)
```
...
cd android && ./gradlew test                             # 466 тестов
...
2. **Собрать APK с `dev` и поставить на телефон.** См. раздел 5 — это
   ...
   после трёх бампов сборочной цепочки. Теперь к этому добавилось второе:
   подключить телефон к двум классам, походить между ними, посмотреть на
   виджет после переключения — и, если под рукой есть телефон с прошлой
   версией, обновить его поверх и убедиться, что класс на месте, а введение
   заново не показывают.
