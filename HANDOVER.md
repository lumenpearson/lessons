...
Ворота:  ruff чисто, mypy чисто, 1277 серверных тестов, 559 Android, обе сборки
...
cd server  && python -m pytest -q                        # 1277 тестов, ~4.5 мин
...
**Осталось незакрытым:** замену (`api/edit.py`, `bot/handlers/content.py`)
можно записать на номер без звонка — в журнал и в уведомление «🔁 Замена» она
уйдёт, а ни один телефон её не нарисует. Отдельного сервиса для замен нет, так
что чинить это — трогать обе оболочки; `can_ring` для этого готов. И паста
одного дня из бота (`handlers/timetable.py`) идёт мимо `apply_timetable`
вообще, то есть без всей этой проверки; недельный импорт идёт через сервис и
в порядке.
