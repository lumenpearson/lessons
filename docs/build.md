...
- **Артефакты живут неделю, а не девяносто дней.** Хранилище на бесплатном
  ...
  на сборке, которая прошла. Так и случилось, и вылечить это из workflow нельзя:
  только удалить старые артефакты или дождаться, пока истекут. Поэтому же
  загрузка отчётов тестов помечена `continue-on-error`: гейт — это
