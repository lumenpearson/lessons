...
Обычный CI (`Actions → CI`) собирает debug на каждом пуше и pull request
(артефакт `app-debug`), а release — на мердже в `main`, на ручном запуске и по
тегу (`app-release-unsigned-key`). R8 и сжатие ресурсов — классический источник
«в debug работало, а в установленном APK нет», поэтому проверка никуда не
делась; она просто стоит на мердже, а не на каждом из пяти пушей, которыми PR
обычно доходит до мерджа. Почему так — в «Минутах Actions» ниже.
