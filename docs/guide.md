...
Установленное приложение открывается не полем для кода, а четырьмя шагами:
...
Сменить класс или выйти из него — «Настройки → Класс». После выхода приложение
снова покажет поле кода, а не четыре экрана заново.
...
| **Неделя** | неделя целиком, с кабинетами, заменами и отменами; день раскрывается |
...
«Виджеты» → «Дневник». Растягивается в обе стороны без ограничения сверху, и содержимое
...
Само по себе приложение только читает. Чтобы добавлять домашку и замены прямо с
телефона, его привязывают к аккаунту в Telegram: **«Настройки → Класс → Telegram»**.
...
Бот — это и есть админка; другой нет. Полное описание — [bot.md](bot.md), здесь то, что
нужно обычному участнику класса.
